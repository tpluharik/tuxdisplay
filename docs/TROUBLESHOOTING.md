...
TuxDisplay 0.4.0 could send invalid native touch state to Mutter and abort GNOME Shell. Upgrade to 0.4.2 or newer:
...
Version 0.4.1 introduced guarded pointer translation. Version 0.4.2 added cached-keyframe recovery for static first frames.

## OpenDisplay shows a black screen

1. Confirm `tuxdisplay status` reports GNOME Wayland mode and a running service.
2. Confirm the package version is at least 0.4.2.
...
If a new connection remains black on 0.4.2, capture the user-service log. The log should show a valid OpenDisplay hello and an active H.264 stream.
